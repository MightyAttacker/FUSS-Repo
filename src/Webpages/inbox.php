...
$stmt = $conn->prepare(SELECT* FROM user_mailbox, LEFTJOIN mailbox ON mailbox.id = user_mailbox.message_id, WHERE user_mailbox.user =? AND user_mailboxes.mailbox = "Out";);
  ...
  <div id="topBanner">
    ...
    <div id="logoutButton">
      <input id="logButton" class="button" type="button"
        onclick="location.href='./loginPages/studentLoginPage.php';" value="Logout" />
    </div>
  </div>
...
<div id="inboxContainer">
    <div id="inboxMessages">
        <table id="inboxTable">
            ...
            <?php
            if($stmt->num_rows > 0){
                $stmt->bind_result($id, $sentto, $sentby, $subject, $message, $created);
                while($stmt->fetch()){
                    echo "<tr>";
                    echo "<td>" . htmlspecialchars($sentfrom) . "</td>";
